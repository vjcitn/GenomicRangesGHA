...
%\VignetteDepends{IRanges, GenomicRangesGHA, pasillaBamSubset, GenomicAlignments, TxDb.Dmelanogaster.UCSC.dm3.ensGene}
...
\begin{frame}[fragile]
  ...
  \begin{enumerate}
    \item Load the \Rpackage{GenomicRangesGHA} package.
...
\begin{frame}[fragile]
  \frametitle{Other resources}
    \begin{itemize}
      ...
      \item Vignettes in the \Rpackage{GenomicRangesGHA} package
            (\Rcode{browseVignettes("GenomicRangesGHA")}).

      \item \Rclass{GRanges} and \Rclass{GRangesList} man pages in the
            \Rpackage{GenomicRangesGHA} package.
