...
\title{\Biocpkg{GenomicRangesGHA} HOWTOs}
...
operations implemented in the \Biocpkg{GenomicRangesGHA} and related
...
to Genomic Ranges Classes'' vignette located in the \Biocpkg{GenomicRangesGHA}
...
To display the list of vignettes available in the \Biocpkg{GenomicRangesGHA}
package, use \Rcode{browseVignettes("GenomicRangesGHA")}.
