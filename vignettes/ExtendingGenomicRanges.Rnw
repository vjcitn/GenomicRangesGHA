...
% \VignetteDepends{GenomicRangesGHA, VariantAnnotation}
...
\title{Extending \Biocpkg{GenomicRangesGHA}}
...
The goal of \Biocpkg{GenomicRangesGHA} is to provide general containers
...
The \Biocpkg{GenomicRangesGHA} package does not directly solve either of
these problems, because there are no general solutions. However, it is
adaptible to specialized use cases.

\section{The \Rclass{GenomicRangesGHA} abstraction}

Unbeknownst to many, most of the \Rclass{GRanges} implementation is
provided by methods on the \Rclass{GenomicRangesGHA} class, the virtual
parent class of \Rclass{GRanges}. \Rclass{GenomicRangesGHA} methods
...
\Rclass{DelegatingGenomicRangesGHA}. 
